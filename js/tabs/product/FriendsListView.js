...
class FriendsListView extends React.Component {
  ...
  renderSectionHeader() {
    return <ProductsSectionHeader title="Узнавайте покупки друзей" />;
  }
  ...
  renderRow(friend: Friend) {
    return (
      <FriendCell
        friend={friend}
        onPress={() => this.openFriendsBasket(friend)}
      />
    );
  }
  ...
  renderEmptyList(): ?ReactElement {
    return (
      <EmptyProduct
        ...
        text={'Корзина друзей будет видна тут'}>
        <InviteFriendsButton />
      </EmptyProduct>
    );
  }
  ...
  openFriendsBasket(friend: Friend) {
