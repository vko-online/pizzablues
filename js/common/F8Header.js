...
class ItemWrapperIOS extends React.Component {
  ...
  render() {
    ...
    let content;
    const {title, icon, layout, onPress, disabled} = item;

    const localStyles = [styles.itemText, { color }];
    if (disabled) {
      localStyles.push(styles.disabledText);
    }
    ...
    if (disabled) {
      return (
        <View style={[styles.itemWrapper, styles.disabled]}>
          {content}
        </View>
      );
    }
